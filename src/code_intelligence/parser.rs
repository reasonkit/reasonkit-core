...
impl RustParser {
    fn extract_rust_functions(&self, code: &str) -> Result<Vec<FunctionNode>, Error> {
        ...
        for (i, line) in lines.iter().enumerate() {
            if let Some(captures) = regex::Regex::new(r"fn\s+(\w+)\s*\(").unwrap().captures(line) {
    ...
    fn extract_rust_variables(&self, code: &str) -> Result<Vec<VariableNode>, Error> {
        ...
        for (i, line) in lines.iter().enumerate() {
            if line.trim_start().starts_with("let") || line.trim_start().starts_with("let mut") {
                ...
                if let Some(captures) = regex::Regex::new(r"let(?:\s+mut)?\s+(\w+)").unwrap().captures(line) {
    ...
    fn calculate_rust_complexity(&self, code: &str, functions: &[FunctionNode]) -> Result<ComplexityMetrics, Error> {
    ...
    fn calculate_cyclomatic_complexity(&self, code: &str) -> f64 {
        let complexity_keywords = ["if", "else", "else if", "while", "for", "match", "||", "&&", "?"];
    ...
    fn calculate_halstead_complexity(&self, code: &str) -> HalsteadMetrics {
        ...
        for word in word_patterns.find_iter(code) {
            ...
            if !["let", "fn", "struct", "enum", "impl", "if", "else", "while", "for", "match"].contains(&word_str.as_str()) {
        ...
        let volume = if length > 0 { length as f64 * (length as f64).log2() } else { 0.0 };
        let difficulty = if vocabulary > 0 { (operators.len() as f64 / 2.0) * (operands.len() as f64 / vocabulary as f64) } else { 0.0 };
...
impl JavaParser {
    fn extract_java_methods(&self, code: &str) -> Result<Vec<FunctionNode>, Error> {
        ...
        for (i, line) in lines.iter().enumerate() {
            if let Some(captures) = regex::Regex::new(r"(public|private|protected)?\s*(static)?\s*(\w+)\s+(\w+)\s*\(").unwrap().captures(line) {
                ...
                methods.push(FunctionNode {
                    ...
                    body: vec![],
                    visibility: self.parse_java_visibility(&captures[1]),
                    is_async: false,
                    line_number,
                    complexity: ComplexityMetrics::default(),
                });
    ...
    fn extract_java_classes(&self, code: &str) -> Result<Vec<ClassNode>, Error> {
        ...
        for (i, line) in lines.iter().enumerate() {
            if let Some(captures) = regex::Regex::new(r"(public|private|protected)?\s*class\s+(\w+)").unwrap().captures(line) {
    ...
    fn extract_java_variables(&self, code: &str) -> Result<Vec<VariableNode>, Error> {
        ...
        for (i, line) in lines.iter().enumerate() {
            if let Some(captures) = regex::Regex::new(r"(public|private|protected)?\s*(\w+)\s+(\w+)\s*(?:=|;)").unwrap().captures(line) {
    ...
    fn calculate_java_complexity(&self, code: &str, methods: &[FunctionNode]) -> Result<ComplexityMetrics, Error> {
        let lines_of_code = code.lines().count() as u32;
        let cyclomatic_complexity = self.calculate_cyclomatic_complexity(code);
    ...
    fn calculate_cyclomatic_complexity(&self, code: &str) -> f64 {
        let complexity_keywords = ["if", "else", "else if", "while", "for", "switch", "case", "catch", "&&", "||", "?"];
    ...
    fn calculate_halstead_complexity(&self, code: &str) -> HalsteadMetrics {
        ...
        for word in word_patterns.find_iter(code) {
            let word_str = word.as_str().to_string();
            if !["public", "private", "protected", "static", "class", "if", "else", "while", "for", "switch", "case"].contains(&word_str.as_str()) {
                *operands.entry(word_str).or_insert(0) += 1;
            }
        }
...
macro_rules! implement_basic_parser {
    ($parser_name:ident, $language:ident, $features:expr) => {
        impl $parser_name {
            pub fn new() -> Self {
                Self
            }
        ...
        impl LanguageParser for $parser_name {
            async fn parse(&self, code: &str) -> Result<UnifiedAST, Error> {
                info!("Parsing {} code ({} characters)", stringify!($language), code.len());
...
implement_basic_parser!(KotlinParser, ProgrammingLanguage::Kotlin, LanguageFeatures {
...
implement_basic_parser!(ObjectiveCParser, ProgrammingLanguage::ObjectiveC, LanguageFeatures {
...
implement_basic_parser!(TypeScriptParser, ProgrammingLanguage::TypeScript, LanguageFeatures {
...
implement_basic_parser!(JavaScriptParser, ProgrammingLanguage::JavaScript, LanguageFeatures {
...
implement_basic_parser!(PythonParser, ProgrammingLanguage::Python, LanguageFeatures {
