...
impl ValidatingProtocolExecutor {
    ...
    pub fn new() -> Result<Self> {
        Self::with_configs(ExecutorConfig::default(), ValidationExecutorConfig::default())
    }
    ...
    pub async fn execute_with_validation(
        ...
        let base_result = self.base_executor.execute(protocol_id, input.clone()).await?;
        ...
        let validation_result = self.apply_validation(protocol_id, &input, &base_result).await?;
    ...
    pub async fn execute_profile_with_validation(
        ...
        let base_result = self.base_executor.execute_profile(profile_id, input.clone()).await?;
        ...
        // Apply DeepSeek validation
        let validation_result = self.apply_validation(profile_id, &input, &base_result).await?;
    ...
    fn should_validate(&self, protocol_id: &str, result: &ProtocolOutput) -> bool {
        ...
        if !self.validate_protocols.is_empty() && !self.validate_protocols.contains(&protocol_id.to_string()) {
            return false;
        }
        
        if self.skip_protocols.contains(&protocol_id.to_string()) {
            return false;
        }
    ...
    fn merge_validation_results(
        ...
        base_result.confidence = (base_result.confidence * validation_impact).min(1.0).max(0.0);
        
        // Add validation performance metrics
        base_result.tokens.add(&validation_result.tokens_used);
    ...
    pub fn get_profile(&self, id: &str) -> Option<super::profiles::ReasoningProfile> {
        self.base_executor.get_profile(id)
    }
}
