...
//!
//! These tests require external dependencies (Qdrant) and test
//! end-to-end functionality across storage backends.
...
mod integration_tests {
    ...
    mod end_to_end_tests {
        ...
        async fn test_in_memory_full_workflow() {
            ...
            let search_results = storage.search_by_vector(&query_embedding, 2, &context).await.unwrap();
        ...
        async fn test_storage_concurrent_access() {
            ...
            for i in 0..10 {
                ...
                let handle = tokio::spawn(async move {
                    ...
                    let doc = create_test_document(&format!("Concurrent doc {}", i));
                    storage_clone.store_document(&doc, &context).await.unwrap();

                    // Small delay to increase chance of race conditions
                    sleep(Duration::from_millis(1)).await;

                    let retrieved = storage_clone.get_document(&doc.id, &context).await.unwrap();
                    assert!(retrieved.is_some());
                    assert_eq!(retrieved.unwrap().content.raw, doc.content.raw);
                });
                handles.push(handle);
        ...
        async fn test_storage_load_simulation() {
            ...
            for (i, &_doc_id) in doc_ids.iter().enumerate() {
                if i % 2 == 0 { // Add embeddings to even-indexed documents
                    let chunk_id = uuid::Uuid::new_v4();
            ...
            for &doc_id in doc_ids.iter().step_by(3) { // Every 3rd document
                let retrieved = storage.get_document(&doc_id, &context).await.unwrap();
                assert!(retrieved.is_some());
            }

            // Phase 5: Bulk deletion
            for &doc_id in doc_ids.iter().step_by(2) { // Delete every 2nd document
    ...
    mod performance_tests {
        ...
        async fn test_storage_performance_baseline() {
            ...
            assert!(store_duration.as_millis() < 1000, "Storage took too long: {:?}", store_duration);
            assert!(retrieve_duration.as_millis() < 500, "Retrieval took too long: {:?}", retrieve_duration);

            println!("Storage performance - Store: {:?}, Retrieve: {:?}", store_duration, retrieve_duration);
        }
        ...
        async fn test_embedding_performance() {
            ...
            assert!(search_duration.as_millis() < 200, "Search took too long: {:?}", search_duration);

            println!("Embedding search performance: {:?} for 50 searches", search_duration);
        }
    }
    ...
    mod reliability_tests {
        ...
        async fn test_storage_data_integrity() {
            ...
            let retrieved = storage.get_document(&doc.id, &context).await.unwrap().unwrap();
            ...
            let binary_retrieved = storage.get_document(&binary_doc.id, &context).await.unwrap().unwrap();
    ...
    // Note: Qdrant integration tests would go here but require a running Qdrant instance
    // They would test the full QdrantStorage implementation with actual database operations
}