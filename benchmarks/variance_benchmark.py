...
def run_full_benchmark(
    ...
    results = []
    for q in questions:
        result = run_variance_test(client, q, n_runs, model, temperature, verbose)
...
def print_summary(summary: BenchmarkSummary):
    ...
    for r in summary.question_results:
        ...
        print(
            f"  Structured: {r['structured_unique_answers']} unique answers, {struct_agree:.0f}% agreement"
        )
        print(
            f"  Raw correct: {r['raw_correct_rate']*100:.0f}%, Structured correct: {r['structured_correct_rate']*100:.0f}%"
        )
...
def main():
    parser = argparse.ArgumentParser(description="ReasonKit Variance Reduction Benchmark")
    parser.add_argument(
        "--runs", "-n", type=int, default=20, help="Number of runs per question (default: 20)"
    )
    parser.add_argument(
        "--model",
        "-m",
    ...
    parser.add_argument(
        "--temperature", "-t", type=float, default=0.0, help="Temperature setting (default: 0.0)"
    )
    parser.add_argument(
        "--output-dir",
        "-o",
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Quick test with 5 runs and 3 questions"
    )
